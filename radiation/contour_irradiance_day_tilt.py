...
betas = []*ureg.deg
gammas = []*ureg.deg
module_type='1daxisNS'
rbNS = Radiation(latitudes, days, betas, gammas, module_type)

line = plt.plot(days.magnitude, np.squeeze(rbNS.I_tmD.magnitude),color=colors[3])
line_objects += line

module_type='1daxisEW'
rbEW = Radiation(latitudes, days, betas, gammas, module_type)

line = plt.plot(days.magnitude, np.squeeze(rbEW.I_tmD.magnitude),color=colors[4])
line_objects += line
rbEW = Radiation(latitudes, days, betas, gammas, module_type)

module_type='2d'
rb2d = Radiation(latitudes, days, betas, gammas, module_type)
line = plt.plot(days.magnitude,np.squeeze(rb2d.I_tD.magnitude),color=colors[5])
line_objects += line

labels += ['1D Tracking NS', '1D Tracking EW', '2D Tracking']
